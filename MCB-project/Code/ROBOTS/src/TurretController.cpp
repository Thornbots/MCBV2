    ...
    double dt)
{
    if (turretControllerTimer.execute())
    {
        ...
        yawMotorVoltage =
            getYawVoltage(driveTrainRPM, yawAngleRelativeWorld, yawRPM, desiredYawAngle, dt);
    }
...
int TurretController::getYawVoltage(
    double driveTrainRPM,
    double yawAngleRelativeWorld,
    double yawRPM,
...
void TurretController::disable() { robotDisabled = true; }
void TurretController::enable() { robotDisabled = false; }
