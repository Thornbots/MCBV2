...
}  // Watch this cute video of a cat instead: https://youtu.be/hg3e1KflmC8
...
/**
 * Updates the values of the motors' speeds. i.e. if you're beyblading, it will tell the
 * motor_yaw_speed to change depending on what needs to change
 */
void TurretController::setMotorValues(
    bool useWASD,
    double angleOffset,
    double right_stick_vert,
    double right_stick_horz,
    int motor_one_speed,
    int motor_four_speed,
    int16_t wheel_value,
    bool isRightStickUp,
    bool isLeftStickUp,
    ...
    int leftSwitchValue)
{
    ...
    motor_yaw_speed = getYawMotorSpeed(
        angleOffset,
        motor_one_speed,
        motor_four_speed,
        isRightStickUp,
        ...
        psuedo_right_switch);
    motor_pitch_speed =
        getPitchMotorSpeed(useWASD, right_stick_vert, (right_stick_vert * ((double)20.0)));
...
void TurretController::setMotorSpeeds(bool sendMotorTimeout)
{
    ...
    // Pitch Motor
    motor_pitch.setDesiredOutput(static_cast<int32_t>(
        motor_pitch_speed -
        6000.0f));  // The ~6000.0f is the feed forward (counters gravity for the pitch motor)
...
void TurretController::startShooting() { isShooting = true; }

void TurretController::stopShooting() { isShooting = false; }

/**
 * TODO: Make this functional!
 * Want an exponentional growth as the value is around 2-3, but also want a very weak response
 * around 0-2. (ish. these numbers are *magical* numbers)
 */
...
/**
 * Returns the speed that the yaw motor needs to go to counterbalance the effect of beyblading or
 * turning. Currently only works when in beyBlading mode.
 * TODO: Make this spin the yawMotor dependent on mouse(only when NOT using CV)
 */
int TurretController::getYawMotorSpeed(
    double actualAngle,
    int motor_one_speed,
    int motor_four_speed,
    bool isRightStickUp,
...
void TurretController::reZero() { current_yaw_angle = 0; }
};  // namespace ThornBots