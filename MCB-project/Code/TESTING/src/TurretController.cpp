...
namespace ThornBots {
    GimbalSubsystem::GimbalSubsystem(tap::Drivers* m_driver) {
    ...
    GimbalSubsystem::~GimbalSubsystem() {}  // Watch this cute video of a cat instead: https://youtu.be/hg3e1KflmC8

    void GimbalSubsystem::FollowDriveTrain() {
        // TODO - Make this work
        desiredAngle = 0;
    ...
    double GimbalSubsystem::getAngleOffset() {
    ...
    float GimbalSubsystem::getYawEncoderAngle() { return tap::motor::DjiMotor::encoderToDegrees(motor_yaw.getEncoderWrapped()); }

    /**
     * Updates the values of the motors' speeds. i.e. if you're beyblading, it will tell the motor_yaw_speed to change depending on what needs to
     * change
     */
    void GimbalSubsystem::setMotorValues(bool useWASD, double angleOffset, double right_stick_vert, double right_stick_horz, int motor_one_speed,
                                         int motor_four_speed, int16_t wheel_value, bool isRightStickUp, bool isLeftStickUp, int rightSwitchValue,
                                         int leftSwitchValue) {
        ...
        motor_yaw_speed = getYawMotorSpeed(angleOffset, motor_one_speed, motor_four_speed, isRightStickUp, isLeftStickUp, right_stick_horz,
                                           right_stick_vert, psuedo_right_switch);
     ...
     */
    void GimbalSubsystem::setMotorSpeeds(bool sendMotorTimeout) {
        if (!sendMotorTimeout) {
            return;
        }
        ...
        // Pitch Motor
        motor_pitch.setDesiredOutput(
            static_cast<int32_t>(motor_pitch_speed - 6000.0f));  // The ~6000.0f is the feed forward (counters gravity for the pitch motor)
     ...
     * sendMotorTimeout should be the method call sendMotorTimeout.execute() when calling this method
     */
    void GimbalSubsystem::stopMotors(bool sendMotorTimeout) {
        if (!sendMotorTimeout) {
            return;
        }
        motor_yaw_speed = 0;
    ...
    void GimbalSubsystem::startShooting() { isShooting = true; }

    void GimbalSubsystem::stopShooting() { isShooting = false; }

    /**
     * TODO: Make this functional!
     * Want an exponentional growth as the value is around 2-3, but also want a very weak response around 0-2. (ish. these numbers are *magical*
     * numbers)
     */
    int GimbalSubsystem::homemadePID(double value) {
     ...
     */
    int GimbalSubsystem::getYawMotorSpeed(double actualAngle, int motor_one_speed, int motor_four_speed, bool isRightStickUp, bool isLeftStickUp,
                                          double right_stick_horz, double right_stick_vert, int rightSwitchValue) {
    ...
    int GimbalSubsystem::getPitchMotorSpeed(bool useWASD, double right_stick_vert, double target_angle) {
    ...
    int GimbalSubsystem::getIndexerMotorSpeed(int16_t wheel_value) {
        if (wheel_value > 1.0) {
            return 1500;
    ...
    int GimbalSubsystem::getFlywheelsSpeed(int16_t wheel_value) {
    ...
    void GimbalSubsystem::reZero() { current_yaw_angle = 0; }
};  // namespace ThornBots