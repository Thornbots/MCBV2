...
namespace ThornBots {
    DrivetrainSubsystem::DrivetrainSubsystem(tap::Drivers* m_driver) {
        // TODO
    ...
    void DrivetrainSubsystem::DriveTrainMovesTurretFollow(double turnSpeed, double translationSpeed, double translationAngle) {
        convertTranslationSpeedToMotorSpeeds(translationSpeed, translationAngle);
        adjustMotorSpeedWithTurnSpeed(turnSpeed);
    }

    void DrivetrainSubsystem::TurretMovesDriveTrainFollow(double translationSpeed, double translationAngle, double driveTrainAngleFromTurret) {
    ...
    void DrivetrainSubsystem::TurretMovesDriveTrainIndependent(double translationSpeed, double translationAngle, double driveTrainAngleFromTurret) {
        convertTranslationSpeedToMotorSpeeds(translationSpeed, translationAngle);
    }

    void DrivetrainSubsystem::setMotorSpeeds(bool sendMotorTimeout) {
        if (sendMotorTimeout) {
    ...
    void DrivetrainSubsystem::stopMotors(bool sendMotorTimeout) {
        motor_one_speed = 0;
        motor_two_speed = 0;
        motor_three_speed = 0;
        motor_four_speed = 0;
     ...
     * direction)
     */
    double DrivetrainSubsystem::getAngle(double xPosition, double yPosition) {
        double angle = 0;
        if (xPosition == 0) {
            if (yPosition == 0) {
                return angle;
     ...
     */
    double DrivetrainSubsystem::getMagnitude(double xPosition, double yPosition) { return sqrt((xPosition * xPosition) + (yPosition * yPosition)); }
     ...
     */
    double DrivetrainSubsystem::getScaledQuadratic(double magnitude) { return pow(magnitude, 2); }
     ...
     */
    void DrivetrainSubsystem::setMotorValues(double right_stick_vert, double right_stick_horz, double left_stick_vert, double left_stick_horz,
                                             float yaw_angle, int rightSwitchState, int leftSwitchValue) {
     ...
     */
    double DrivetrainSubsystem::updateMotorSpeeds(double MotorNewSpeed, double MotorCurrentSpeed, int slewRate) {
     ...
     */
    int DrivetrainSubsystem::getMotorSetOneTranslatingSpeed(double xPosition, double yPosition) {
     ...
     * https://seamonsters-2605.github.io/archive/mecanum/
     */
    int DrivetrainSubsystem::getMotorSetTwoTranslatingSpeed(double xPosition, double yPosition) {
        float refined_angle = yaw_motor_angle + REFINED_ANGLE_OFFSET;
        if (refined_angle < 0) refined_angle += 360.0f;
        double angle = getAngle(xPosition, yPosition) + (PI * refined_angle / 180.0f);
        if (lockDrivetrain) angle = yaw_motor_angle;
    ...
    int DrivetrainSubsystem::getMotorOneSpeedWithCont(bool isBeyblading, double right_stick_vert, double right_stick_horz, double left_stick_vert,
    ...
    int DrivetrainSubsystem::getMotorTwoSpeedWithCont(bool doBeyblading, double right_stick_vert, double right_stick_horz, double left_stick_vert,
                                                      double left_stick_horz) {
        if (doBeyblading) {
    ...
    int DrivetrainSubsystem::getMotorThreeSpeedWithCont(bool doBeyblading, double right_stick_vert, double right_stick_horz, double left_stick_vert,
                                                        double left_stick_horz) {
        if (doBeyblading) {
    ...
    int DrivetrainSubsystem::getMotorFourSpeedWithCont(bool doBeyblading, double right_stick_vert, double right_stick_horz, double left_stick_vert,
                                                       double left_stick_horz) {
        if (doBeyblading) {
