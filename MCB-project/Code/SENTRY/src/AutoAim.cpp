...
const double deltaTime = 0.03;  // Frame time
...
void computeFiniteDifferences(const std::vector<PanelData>& data, double deltaTime, double& dr_dt, double& d2r_dt2, double& dp_dt, double& d2p_dt2) {
    size_t n = data.size();
    if (n > 2) {
...
GimbalCommand updateAim(double x, double y, double z, double current_pitch, double current_yaw) {
    ...
    command.yaw = fmod(theta_triple_prime + current_yaw, 2 * PI);
