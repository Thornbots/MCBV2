...
{
public:
    modm_always_inline explicit Flash(const T* addr = 0) : address(addr) {}

    template <typename U>
    modm_always_inline explicit Flash(const Flash<U>& rhs) : address((T*)rhs.address)
    {
    }
    ...
    modm_always_inline bool isValid() const { return (address != 0); }
...
private:
    template <typename U>
    friend IOStream& operator<<(IOStream&, const Flash<U>&);
#endif
};
 ...
 * @ingroup modm_architecture_accessor
 */
inline modm::IOStream& operator<<(modm::IOStream& os, modm::accessor::Flash<char> ptr)
{
    char c;
    while ((c = *ptr++))
    {
