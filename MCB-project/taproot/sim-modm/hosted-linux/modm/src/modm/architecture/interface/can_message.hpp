...
struct Message
{
    ...
    inline Message(
        uint32_t inIdentifier,
        uint8_t inLength,
        const uint64_t& inData,
        bool extended = false)
        : identifier(inIdentifier),
          length(std::min(inLength, uint8_t(8)))
    {
        flags.extended = extended;
        const uint8_t* inDataB = reinterpret_cast<const uint8_t*>(&inData);
        for (uint8_t ii = 0; ii < length; ++ii) data[ii] = inDataB[length - ii - 1];
    }

    inline uint32_t getIdentifier() const { return identifier; }

    inline void setIdentifier(uint32_t id) { identifier = id; }

    inline void setExtended(bool extended = true) { flags.extended = (extended) ? 1 : 0; }

    inline bool isExtended() const { return (flags.extended != 0); }

    inline void setRemoteTransmitRequest(bool rtr = true) { flags.rtr = (rtr) ? 1 : 0; }

    inline bool isRemoteTransmitRequest() const { return (flags.rtr != 0); }

    inline uint8_t getLength() const { return length; }

    inline void setLength(uint8_t len) { length = len; }
...
public:
    inline bool operator==(const modm::can::Message& rhs) const
    {
        return (
            (this->identifier == rhs.identifier) and (this->length == rhs.length) and
            (this->flags.rtr == rhs.flags.rtr) and (this->flags.extended == rhs.flags.extended) and
...
inline modm::IOStream& operator<<(modm::IOStream& s, const modm::can::Message& m)
{
    s.printf("id = %04" PRIx32 ", len = ", m.identifier);
    s << m.length;
    s.printf(", flags = %c%c, data = ", m.flags.rtr ? 'R' : 'r', m.flags.extended ? 'E' : 'e');
