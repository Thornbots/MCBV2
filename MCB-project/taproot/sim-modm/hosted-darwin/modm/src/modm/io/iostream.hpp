...
public:
     ...
     * @endcode
     */
    inline IOStream(IODevice& odevice) : device(&odevice) {}

    // Acccessors -------------------------------------------------------------
    inline IOStream& write(char c)
    {
        device->write(c);
        return *this;
    }
    ...
    template <size_t N>
    inline IOStream& get(char (&s)[N])
    {
        return get(s, N);
    }

    // Modes ------------------------------------------------------------------
    inline IOStream& flush()
    {
        device->flush();
        mode = Mode::Ascii;
        return *this;
    ...
    inline IOStream& operator<<(const char& v)
    {
        if (mode == Mode::Ascii)
            device->write(v);
    ...
    inline IOStream& operator<<(const uint8_t& v)
    {
        if (mode == Mode::Ascii)
    ...
    {
        writeIntegerMode(static_cast<uint32_t>(v));
        return *this;
    }
#endif
#endif
    ...
    {
        writeIntegerMode(static_cast<int64_t>(v));
        return *this;
    }
    inline IOStream& operator<<(const long unsigned int& v)
    {
        writeIntegerMode(static_cast<uint64_t>(v));
        return *this;
    }
    inline IOStream& operator<<(const float& v)
    {
        writeFloat(v);
        return *this;
    }

    inline IOStream& operator<<(const double& v)
    {
        writeDouble(v);
        return *this;
    }
    inline IOStream& operator<<(const char* s)
    {
        device->write(s);
        return *this;
    }
    ...
    inline IOStream& operator<<(const void* p)
    {
        writePointer(p);
        return *this;
    }
    ...
    {
        writePointer(reinterpret_cast<const void*>(&pointer));
        return *this;
    }

    inline IOStream& operator<<(IOStream& (*format)(IOStream&)) { return format(*this); }

    // printf -----------------------------------------------------------------
    IOStream& printf(const char* fmt, ...) __attribute__((format(printf, 2, 3)));

    IOStream& vprintf(const char* fmt, va_list vlist) __attribute__((format(printf, 2, 0)));
...
protected:
    ...
    inline void writeFloat(float value) { writeDouble(static_cast<double>(value)); }
...
inline IOStream& flush(IOStream& ios) { return ios.flush(); }

//// Write a newline. **DOES NOT FLUSH THE STREAM!**
inline IOStream& endl(IOStream& ios) { return ios.endl(); }

/// set the output mode to binary style
inline IOStream& bin(IOStream& ios) { return ios.bin(); }

/// set the output mode to hexadecimal style
inline IOStream& hex(IOStream& ios) { return ios.hex(); }

/// set the output mode to ASCII style
inline IOStream& ascii(IOStream& ios) { return ios.ascii(); }

/// Set the foreground colour on ANSI terminals.
IOStream& black(IOStream& ios);

IOStream& red(IOStream& ios);

IOStream& green(IOStream& ios);

IOStream& yellow(IOStream& ios);

IOStream& blue(IOStream& ios);

IOStream& magenta(IOStream& ios);

IOStream& cyan(IOStream& ios);

IOStream& white(IOStream& ios);
/// @}
