...
struct Register
{
    ...
    constexpr bool operator!() const { return not bool(value); }
    /// @}

#if MODM_HAS_IOSTREAM
    /// Printing a register will output its numeric value.
    friend IOStream &operator<<(IOStream &s, const Register<T> &m) { return (s << m.value); }
#endif

protected:
    /// This class is meant to be subclassed
    constexpr Register(UnderlyingType initialValue) : value(initialValue) {}
};
...
struct FlagsOperators : public ::modm::Register<T>
{
    ...
    constexpr FlagsOperators(Enum flag) : Register<T>(T(flag)) {}

    /// When explicitly called, this class constructs itself out of the underlying type
    explicit constexpr FlagsOperators(UnderlyingType flag) : Register<T>(flag) {}

    /// When explicitly called, this class constructs itself out of the superclass
    explicit constexpr FlagsOperators(Register<T> const &o) : Register<T>(o.value) {}
    ...
    {
        return FlagsOperators(value & o.value);
    }
    ...
    /// bitwise **AND** with a single bit
    friend constexpr FlagsOperators operator&(Enum const &a, FlagsOperators const &b)
    {
        return b & a;
    }
    /// bitwise **OR** with a single bit
...
#ifndef __DOXYGEN__
    constexpr FlagsOperators operator&(Enum const &flag) const
    {
        return operator&(FlagsOperators(flag));
    }
    constexpr FlagsOperators operator|(Enum const &flag) const
    {
        return operator|(FlagsOperators(flag));
...
struct Flags : public ::modm::FlagsOperators<Enum, T>
{
    ...
    /// constructs itself out of a strongly-typed enum
    constexpr Flags(Enum flag) : FlagsOperators<Enum, T>(flag) {}

    constexpr Flags(FlagsOperators<Enum, T> const &flag) : FlagsOperators<Enum, T>(flag) {}

    /// When explicitly called, this class constructs itself out of the underlying type
    explicit constexpr Flags(UnderlyingType flag) : FlagsOperators<Enum, T>(flag) {}

    /// When explicitly called, this class constructs itself out of the superclass
    explicit constexpr Flags(Register<T> const &o) : FlagsOperators<Enum, T>(o) {}
    ...
    /// Sets a single bit.
    inline void set(Enum const &flag) { *this |= flag; }
    /// Sets multiple bits.
    inline void set(Flags const &o) { *this |= o; }

    /// Clears a single bit.
    inline void reset(Enum const &flag) { *this &= ~flag; }
    /// Clears multiple bits.
    inline void reset(Flags const &o) { *this &= ~o; }

    /// Toggles a single bit.
    inline void toggle(Enum const &flag) { *this ^= flag; }
    /// Toggles multiple bits.
    inline void toggle(Flags const &o) { *this ^= o; }
    ...
    constexpr bool all(Enum const &flag) const { return bool(*this & flag); }
    /// Returns `true` if bit is set
    constexpr bool any(Enum const &flag) const { return bool(*this & flag); }
    /// Returns `true` if bit is **not** set
    constexpr bool none(Enum const &flag) const { return !(*this & flag); }
    /// @}

    /// @{

    /// Returns `true` if **all** of the passed bits are set
    constexpr bool all(Flags const &o) const { return (*this & o) == o; }
    /// Returns `true` if **any** of the passed bits are set
    constexpr bool any(Flags const &o) const { return bool(*this & o); }
    /// Returns `true` if **none** of the passed bits are set
    constexpr bool none(Flags const &o) const { return (*this & o).value == 0; }
    /// @}
} modm_packed;
...
struct FlagsGroup<T, Ts...> : public FlagsGroup<Ts...>
{
    ...
    constexpr FlagsGroup(
        ::modm::FlagsOperators<typename T::EnumType, typename T::UnderlyingType> value)
...
#endif
{
    ...
    /// copy constructor
    constexpr FlagsGroup(FlagsGroup const &o) : Register<typename T::UnderlyingType>(o.value) {}
...
template <
    typename Parent,
    typename Enum,
    typename Parent::UnderlyingType Mask,
    typename Parent::UnderlyingType Position = 0>
struct Configuration
    : public ::modm::FlagsOperators<typename Parent::EnumType, typename Parent::UnderlyingType>
{
private:
...
public:
    ...
    static inline void reset(Parent &parent) { parent.value &= ~(Mask << Position); }
    ...
    static constexpr Parent mask() { return Parent((Mask << Position)); }

    /// @cond
    constexpr operator Parent() const { return Parent(FlagsOperators<EType, UType>::value); }
    /// @endcond
} modm_packed;
...
template <
    typename Parent,
    typename Parent::UnderlyingType Width,
    typename Parent::UnderlyingType Position = 0>
struct Value
    : public ::modm::FlagsOperators<typename Parent::EnumType, typename Parent::UnderlyingType>
{
...
public:
    ...
    static inline void reset(Parent &parent) { parent.value &= ~Mask; }

    /// returns the value from a Flags register
    static constexpr UType get(Parent const &parent) { return (parent.value & Mask) >> Position; }
    /// @}

    /// returns the shifted mask for this value
    static constexpr Parent mask() { return Parent(Mask); }
...
template <typename Enum>
constexpr ::modm::Flags<Enum> operator|(Enum const &a, Enum const &b) { return
::modm::Flags<Enum>(a) | b;  } template <typename Enum> constexpr ::modm::Flags<Enum> operator&(Enum
const &a, Enum const &b) { return ::modm::Flags<Enum>(a) & b; } template <typename Enum> constexpr
::modm::Flags<Enum> operator^(Enum const &a, Enum const &b) { return ::modm::Flags<Enum>(a) ^ b; }
//*/
 ...
 * @hideinitializer
 */
#define MODM_TYPE_FLAGS(Parent) MODM_INTERNAL_FLAGS(Parent, )
...
#define MODM_FLAGS_CONFIG(Parent, Config)                  \
    using MODM_CONCAT(Config, _t) = ::modm::Configuration< \
        MODM_CONCAT(Parent, _t),                           \
        Config,                                            \
        MODM_CONCAT(Parent, _t)::UnderlyingType(MODM_CONCAT(Parent::Config, _Mask))>

/// @cond
#define MODM_INT_TYPE_FLAGS(Parent) MODM_INTERNAL_FLAGS(Parent, friend)
...
#define MODM_INTERNAL_FLAGS_EOP(type, Parent, op, scope)                     \
    scope constexpr Parent operator op(                                      \
        typename Parent::EnumType const &lhs,                                \
        typename Parent::EnumType const &rhs)                                \
    {                                                                        \
