...
public:
    ...
    constexpr HsvT(const HsvT<U>& hsv_other)
        : hue(hsv_other.hue << 8),
          saturation(hsv_other.saturation << 8),
          value(hsv_other.value << 8)
    {
    }
     ...
     */
    template <std::unsigned_integral U>
    constexpr HsvT(const BrightnessT<U> gray) : hue(0),
                                                saturation(0),
                                                value(gray.value)
    {
    }
...
private:
    template <std::unsigned_integral U>
    friend IOStream& operator<<(IOStream&, const HsvT<U>&);
};
...
template <std::unsigned_integral U>
IOStream& operator<<(IOStream& os, const color::HsvT<U>& color)
{
