...
public:
    ...
    constexpr RgbT(const RgbT<U>& rgb_other)
        : red(rgb_other.red << 8),
          green(rgb_other.green << 8),
          blue(rgb_other.blue << 8)
    {
    }
...
private:
    template <std::unsigned_integral U>
    friend IOStream& operator<<(IOStream&, const RgbT<U>&);
};
...
template <
    ...
    typename IntermediateType = float,
    std::unsigned_integral ReturnType = T>
requires std::is_fundamental_v<IntermediateType>
constexpr RgbT<ReturnType> normalizeColor(RgbT<T> rgb, IntermediateType multiplier = 1)
{
...
template <std::unsigned_integral U>
IOStream& operator<<(IOStream& os, const color::RgbT<U>& color)
{
