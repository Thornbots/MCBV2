...
concept compatible_value_type =
    !is_any_type<std::remove_cvref_t<T>>::value &&
    !is_inplace_type<std::remove_cvref_t<T>>::value && std::copy_constructible<T> &&
...
private:
    ...
    static void copy(const std::byte* data, std::byte* other_data, HandlerFunc* other_handler_ptr);
};
...
public:
    ...
    requires inplace_any_impl::compatible_value_type<T> inplace_any(
...
private:
    ...
    template <typename T, std::size_t S>
    friend auto any_cast(inplace_any<S>& any) noexcept -> std::remove_cvref_t<T>&;
 ...
 */
template <typename T, std::size_t Size>
auto any_cast(inplace_any<Size>* any) noexcept -> std::add_pointer_t<std::remove_cvref_t<T>>;
 ...
 */
template <typename T, std::size_t Size>
auto any_cast(const inplace_any<Size>& any) noexcept -> std::add_const_t<std::remove_cvref_t<T>>&;
 ...
 */
template <typename T, std::size_t Size>
auto any_cast(inplace_any<Size>& any) noexcept -> std::remove_cvref_t<T>&;
...
template <std::size_t Size>
auto inplace_any<Size>::operator=(inplace_any&& other) noexcept -> inplace_any&
{
    if (this != &other)
    {
        destruct();
...
template <std::size_t Size>
template <std::size_t OtherSize>
auto inplace_any<Size>::operator=(inplace_any<OtherSize>&& other) noexcept -> inplace_any&
{
    static_assert(OtherSize <= Size, "cannot assign from larger inplace_any");
    destruct();
    move_from(std::move(other));
...
template <std::size_t Size>
template <typename T, typename... Args>
requires inplace_any_impl::compatible_value_type<T> inplace_any<Size>::inplace_any(
    std::in_place_type_t<T>,
    Args&&... args)
{
...
template <std::size_t Size>
template <typename T>
requires inplace_any_impl::compatible_value_type<T> inplace_any<Size>::inplace_any(T&& value)
{
...
auto inplace_any<Size>::operator=(T&& value) -> inplace_any&
{
    static_assert(sizeof(std::decay_t<T>) <= Size, "type exceeds capacity");
    destruct();
    construct<std::decay_t<T>>(std::forward<T>(value));
...
template <std::size_t Size>
template <typename T, typename... Args>
requires inplace_any_impl::compatible_value_type<T> T& inplace_any<Size>::emplace(Args&&... args)
{
    static_assert(sizeof(T) <= Size, "type exceeds capacity");
    destruct();
...
T& inplace_any<Size>::construct(Args&&... args)
{
    ...
    handler_t::construct(
        reinterpret_cast<std::byte*>(&storage_),
        &handler_,
...
void inplace_any<Size>::move_from(inplace_any<OtherSize>&& other) noexcept
{
    ...
    {
        other.handler_(
            inplace_any_impl::Action::Move,
            reinterpret_cast<std::byte*>(&other.storage_),
            reinterpret_cast<std::byte*>(&storage_),
            reinterpret_cast<void (**)()>(&handler_));
    }
}
...
void inplace_any<Size>::destruct() noexcept
{
    ...
    {
        handler_(
            inplace_any_impl::Action::Destruct,
            reinterpret_cast<std::byte*>(&storage_),
            nullptr,
            nullptr);
        handler_ = nullptr;
...
template <typename T>
void inplace_any_handler<T>::execute(
    Action action,
    std::byte* data,
    std::byte* other_data,
    void (**other_handler_ptr)())
{
...
template <typename T>
void inplace_any_handler<T>::move(
    std::byte* data,
    std::byte* other_data,
    HandlerFunc* other_handler_ptr) noexcept
{
...
template <typename T>
void inplace_any_handler<T>::copy(
    const std::byte* data,
    std::byte* other_data,
