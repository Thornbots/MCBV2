...
public:
    ...
    bool inline isResumableRunning() const { return !isStoppedRf(); }

    /// @return the nesting depth in the current resumable function, or -1 if called outside any
    /// resumable function
    int8_t inline getResumableDepth() const { return static_cast<int8_t>(rfLevel) - 1; }
...
protected:
    ...
    rf::State inline pushRf(uint8_t /*index*/) { return rfStateArray[rfLevel++]; }

    /// always call this before returning from the run function!
    /// decreases nesting level
    void inline popRf() { rfLevel--; }

    // invalidates the parent nesting level
    // @warning	be aware in which nesting level you call this! (before popRf()!)
    void inline stopRf(uint8_t /*index*/) { rfStateArray[rfLevel - 1] = rf::Stopped; }

    /// sets the state of the parent nesting level
    /// @warning	be aware in which nesting level you call this! (before popRf()!)
    void inline setRf(rf::State state, uint8_t /*index*/) { rfStateArray[rfLevel - 1] = state; }
    ...
    bool inline nestingOkRf() const
    {
        return modm_assert_continue_fail_debug(
            rfLevel < Levels,
            "rf.nest",
            "Called too many nested ResumableFunctions of the same class!",
            this);
    }

    /// @return	`true` if `stopRf()` has been called before
    bool inline isStoppedRf() const { return (rfStateArray[rfLevel] == rf::Stopped); }
    ...
    static void checkRfType()
    {
        static_assert(
            isNested == true,
            "You cannot declare an index for a _nested_ resumable function!");
    }
    /// @endcond
private:
...
{
protected:
    NestedResumable() : rfLevel(-1), rfState(rf::Stopped) {}

public:
    void inline stopResumable() { rfState = rf::Stopped; }

    bool inline isResumableRunning() const { return !isStoppedRf(); }

    int8_t inline getResumableDepth() const { return rfLevel; }

protected:
    rf::State inline pushRf(uint8_t /*index*/)
    {
        rfLevel = 0;
        return rfState;
    }

    void inline popRf() { rfLevel = -1; }

    void inline stopRf(uint8_t /*index*/) { rfState = rf::Stopped; }

    bool inline nestingOkRf() const { return (rfLevel != 0); }

    void inline setRf(rf::State state, uint8_t /*index*/) { rfState = state; }

    bool inline isStoppedRf() const { return (rfState == rf::Stopped); }
