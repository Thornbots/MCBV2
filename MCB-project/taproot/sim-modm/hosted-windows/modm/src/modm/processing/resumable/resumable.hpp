...
enum ResultState
{
    ...
    // reasons to wait
    WrongState =
        127,  ///< A conflicting nested resumable function of the same class is already running
...
struct ResumableResult
{
    ...
    inline uint_fast8_t getState() { return state; }
...
struct ResumableResult<void>
{
    ...
    inline uint_fast8_t getResult() { return state; }
 ...
 * @tparam	Functions	the number of resumable functions that are available in this class.
 * Must be non-zero!
 */
...
public:
    ...
    inline bool stopResumable(uint8_t id)
    {
        if (id < Functions)
        {
    ...
    bool inline isResumableRunning(uint8_t id) const
    {
    ...
    bool inline areAnyResumablesRunning() const
    {
        for (const rf::State state : rfStateArray)
        {
            if (state != rf::Stopped) return true;
        }
        return false;
    }

    /// @return	`true` if any of the specified resumable function are running, else `false`
    bool inline areAnyResumablesRunning(std::initializer_list<uint8_t> ids) const
    {
        for (uint8_t id : ids)
        {
            if (isResumableRunning(id)) return true;
        }
        return false;
    }

    /// @return	`true` if all of the specified resumable function are running, else `false`
    bool inline areAllResumablesRunning(std::initializer_list<uint8_t> ids) const
    {
        for (uint8_t id : ids)
        {
            // = not isResumableRunning(id)
            if (id >= Functions or rfStateArray[id] == rf::Stopped) return false;
        }
        return true;
    }

    /// @return	`true` if none of the specified resumable function are running, else `false`
    bool inline joinResumables(std::initializer_list<uint8_t> ids) const
    {
     ...
     */
    modm::ResumableResult<ReturnType> resumable(...);
    /// @endcond
#endif
...
protected:
    ...
    rf::State modm_always_inline pushRf(uint_fast8_t index) const { return rfStateArray[index]; }

    /// always call this before returning from the run function!
    /// decreases nesting level
    void modm_always_inline popRf() const {}

    // invalidates the parent nesting level
    // @warning	be aware in which nesting level you call this! (before popRf()!)
    void modm_always_inline stopRf(uint_fast8_t index) { rfStateArray[index] = rf::Stopped; }

    /// sets the state of the parent nesting level
    /// @warning	be aware in which nesting level you call this! (before popRf()!)
    void modm_always_inline setRf(rf::State state, uint_fast8_t index)
    {
        rfStateArray[index] = state;
    }

    bool modm_always_inline nestingOkRf() const { return true; }

    /// @return	`true` if `stopRf()` has been called before
    bool modm_always_inline isStoppedRf(uint_fast8_t index) const
    {
    ...
    static void checkRfFunctions()
    {
        static_assert(
            index < Functions,
            "Index out of bounds! Increase the `Functions` template argument of your Resumable "
            "class.");
    }

    /// asserts that this method is called in this parent class
    template <bool isNested>
    static void checkRfType()
    {
