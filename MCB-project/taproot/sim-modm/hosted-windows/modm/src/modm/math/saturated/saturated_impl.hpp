...
template <typename T>
Saturated<T>::Saturated(const T& initialValue) : value(initialValue)
{
}

template <typename T>
void Saturated<T>::setValue(WideType in)
{
...
template <typename T>
Saturated<T>& Saturated<T>::operator+=(const Saturated& other)
{
    WideType temp = static_cast<WideType>(this->value) + static_cast<WideType>(other.value);
    setValue(temp);
...
template <>
void Saturated<uint8_t>::absolute()
{
}

template <>
void Saturated<uint16_t>::absolute()
{
}

template <>
void Saturated<uint32_t>::absolute()
{
}
}  // namespace modm
