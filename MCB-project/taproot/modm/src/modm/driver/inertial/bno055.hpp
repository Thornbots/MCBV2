...
struct bno055
{
    ...
    static constexpr uint8_t addr(uint8_t offset = 1) { return 0x28 + (offset & 0b1); };
...
public:
    ...
    enum class SystemClockStatus : uint8_t
    {
    ...
    enum class SystemTrigger : uint8_t
    {
    ...
    enum class MagDataOutputRate : uint8_t
    {
        Hz2 = 0,
        Hz6 = 1,
        Hz8 = 2,
    ...
    enum class GyrBandwidth : uint8_t
    {
        Hz523 = (0 << 3),
        Hz230 = (1 << 3),
        Hz116 = (2 << 3),
    ...
    /// GYR_Range reset value is 0x00
    enum class GyrPowerMode : uint8_t
    {
        Normal = 0,
        FastPowerUp = 1,
        DeepSuspend = 2,
...
public:
    struct modm_packed Data
    {
        ...
        inline Quaternion<float> quaternion() const
        {
            return Quaternion<float>(r.quaternion).scale(1l << 14);
        }
        inline Vector3f linear_acceleration() const
        {
            return Vector3f(r.linear_acceleration).scaled(1.f / 100);
        }
...
public:
    ...
    inline modm::ResumableResult<bool> configure(OperationMode mode = OperationMode::NDOF)
    {
        return updateRegister(Register::OPR_MODE, mode);
    }

    inline modm::ResumableResult<bool> readData()
    {
        return readRegister(Register::ACCEL_DATA_X_LSB, (uint8_t *)&data.r, Data::size);
    }

    inline modm::ResumableResult<bool> enableExternalClock()
    {
        return updateRegister(Register::SYS_TRIGGER, SystemTrigger::CLK_SEL, Registers_t(0));
    }

public:
    inline modm::ResumableResult<bool> updateRegister(
        Register reg,
        Registers_t setMask,
        Registers_t clearMask = Registers_t(0xff))
    {
        RF_BEGIN();
    ...
    inline modm::ResumableResult<bool> readRegister(
        Register reg,
        uint8_t *output,
        size_t length = 1)
    {
...
protected:
    inline modm::ResumableResult<bool> setPageId(Register regi)
    {
