...
class GenericPrescaler
{
public:
    struct Result
    {
        constexpr Result(T input_frequency, T desired_frequency, uint32_t index, uint32_t prescaler)
            : index{index},
              prescaler{prescaler},
...
public:
    ...
    static constexpr Result from_iterator(
        T input_frequency,
        T desired_frequency,
        Iterator prescalers)
    {
        ...
        {
            prescaler_ceiling = prescaler;
            ii_ceiling++;
            if (prescaler_floor <= desired and desired < prescaler_ceiling) break;
            prescaler_floor = prescaler;
            ii_floor++;
        }
    ...
    static constexpr Result from_list(
        T input_frequency,
        T desired_frequency,
    ...
    static constexpr Result from_function(
        T input_frequency,
        T desired_frequency,
        uint32_t begin,
        uint32_t end,
        Function prescaler_modifier)
    {
        struct linear_range_iterator
        {
            ...
            constexpr bool operator!=(const linear_range_iterator& o) const
            {
        ...
        struct linear_range
        {
            ...
            {
                return linear_range_iterator{m_begin, modifier};
            }
            constexpr linear_range_iterator end() const
            {
                return linear_range_iterator{m_end, modifier};
            }
    ...
    /// @note the range end is *inclusive*: [begin, end].
    static constexpr Result from_range(
        T input_frequency,
        T desired_frequency,
        uint32_t begin,
        uint32_t end)
    ...
    static constexpr Result from_power(
        T input_frequency,
        T desired_frequency,
        uint32_t begin,
        uint32_t end)
