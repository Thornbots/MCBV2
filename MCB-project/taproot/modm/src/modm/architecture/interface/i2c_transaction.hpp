...
public:
    struct Starting
    {
         ...
         */
        Starting(uint8_t address, OperationAfterStart next) : address(address), next(next) {}
...
public:
    ...
    I2cTransaction(uint8_t address) : address(address << 1), state(TransactionState::Idle) {}

    ///	@param	address	the slave address not yet shifted left (address < 128).
    void inline setAddress(uint8_t address) { this->address = address << 1; }
    ...
    bool isBusy() const { return (state == TransactionState::Busy); }
...
public:
    ...
    {
        if (state == TransactionState::Busy) return false;
    ...
    {
        state =
            (cause == DetachCause::NormalStop) ? TransactionState::Idle : TransactionState::Error;
    }
...
public:
    ...
    bool modm_always_inline configurePing() { return configureWriteRead(nullptr, 0, nullptr, 0); }

    bool inline configureWriteRead(
        const uint8_t *writeBuffer,
        std::size_t writeSize,
        uint8_t *readBuffer,
        std::size_t readSize)
    {
        if (state != TransactionState::Busy)
        {
             ...
             *
             * 2 and 3 seem easy too, simply check if writeSize or readSize is non-zero
             * respectively. No? NO! We want to keep the writeSize and readSize persistent, so that
             * you do not need to reconfigure the adapter if nothing changed.
             *
             * So assume the following sequence with both writeSize and readSize non-zero:
             * start - write - restart - read - ...
             *
             * During the first `starting()`, we check `writeSize != 0`, and request next operation
             * `Write`. Then, during the next restart we will check `writeSize != 0`, but because it
             * has not changed, we will never be able to enter the reading phase. We cannot check
             * `readSize != 0` first either, because we encounter the same problem.
             *
             * Ok, so we use `isReading` here to signal, whether we want to write or read during
             * `starting()`.
             */
     ...
     *          `false` otherwise
     */
    bool modm_always_inline configureWrite(const uint8_t *buffer, std::size_t size)
    {
     ...
     *          `false` otherwise
     */
    bool modm_always_inline configureRead(uint8_t *buffer, std::size_t size)
    {
...
protected:
    ...
    Writing writing() override
    {
         ...
         * The code shown in `starting()` does work, but only for the specific sequence of this
         * adapter: `start - write - restart - read - stop`
         *
         * The code starts to break once you have the sequence:
         *     `start - write - write - ...`
         * because during the second write `isReading` is suddenly true and we either Stop or start
         * Reading.
         *
         ...
         *
         * This would be implemented as such (prepending 2 address bytes to a block of data taken
         * from I2cEeprom):
         *
         *     if (writeAddress) {
         *         writeAddress = false;
         ...
         *
         * This would call `starting()` twice, without modifying `isReading` until this method is
         * called.
         */
    ...
    Reading reading() override { return Reading(readBuffer, readSize, OperationAfterRead::Stop); }
...
{
public:
    I2cWriteTransaction(uint8_t address) : I2cTransaction(address), size(0), buffer(nullptr) {}

    bool modm_always_inline configurePing() { return configureWrite(nullptr, 0); }

    bool modm_always_inline configureWriteRead(const uint8_t *, std::size_t, uint8_t *, std::size_t)
    {
        return false;
    }

    bool inline configureWrite(const uint8_t *buffer, std::size_t size)
    {
        if (state != TransactionState::Busy)
        {
            this->buffer = buffer;
    ...
    bool modm_always_inline configureRead(uint8_t *, std::size_t) { return false; }
...
protected:
    ...
    Writing writing() override { return Writing(buffer, size, OperationAfterWrite::Stop); }

    Reading reading() override { return Reading(nullptr, 0, OperationAfterRead::Stop); }
...
{
public:
    I2cReadTransaction(uint8_t address) : I2cTransaction(address), size(0), buffer(nullptr) {}

    bool modm_always_inline configurePing() { return configureRead(nullptr, 0); }
    ...
    bool modm_always_inline configureWrite(uint8_t *, std::size_t) { return false; }

    bool inline configureRead(uint8_t *buffer, std::size_t size)
    {
        if (state != TransactionState::Busy)
        {
            this->buffer = buffer;
...
protected:
    ...
    Writing writing() override { return Writing(nullptr, 0, OperationAfterWrite::Stop); }

    Reading reading() override { return Reading(buffer, size, OperationAfterRead::Stop); }
