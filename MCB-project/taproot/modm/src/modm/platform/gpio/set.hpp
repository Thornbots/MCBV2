...
public:
    ...
    {
        if constexpr (mask(0))
            GPIOA->MODER = (GPIOA->MODER & ~mask2(0)) | mask2(0, i(Mode::Output));
        if constexpr (mask(1))
            GPIOB->MODER = (GPIOB->MODER & ~mask2(1)) | mask2(1, i(Mode::Output));
        if constexpr (mask(2))
            GPIOC->MODER = (GPIOC->MODER & ~mask2(2)) | mask2(2, i(Mode::Output));
        if constexpr (mask(3))
            GPIOD->MODER = (GPIOD->MODER & ~mask2(3)) | mask2(3, i(Mode::Output));
        if constexpr (mask(4))
            GPIOE->MODER = (GPIOE->MODER & ~mask2(4)) | mask2(4, i(Mode::Output));
        if constexpr (mask(5))
            GPIOF->MODER = (GPIOF->MODER & ~mask2(5)) | mask2(5, i(Mode::Output));
        if constexpr (mask(6))
            GPIOG->MODER = (GPIOG->MODER & ~mask2(6)) | mask2(6, i(Mode::Output));
        if constexpr (mask(7))
            GPIOH->MODER = (GPIOH->MODER & ~mask2(7)) | mask2(7, i(Mode::Output));
        if constexpr (mask(8))
            GPIOI->MODER = (GPIOI->MODER & ~mask2(8)) | mask2(8, i(Mode::Output));
    }
    ...
    {
        if constexpr (mask(0))
            GPIOA->BSRR = ((uint32_t(mask(0)) & ~inverted(0)) << 16) | inverted(0);
        if constexpr (mask(1))
            GPIOB->BSRR = ((uint32_t(mask(1)) & ~inverted(1)) << 16) | inverted(1);
        if constexpr (mask(2))
            GPIOC->BSRR = ((uint32_t(mask(2)) & ~inverted(2)) << 16) | inverted(2);
        if constexpr (mask(3))
            GPIOD->BSRR = ((uint32_t(mask(3)) & ~inverted(3)) << 16) | inverted(3);
        if constexpr (mask(4))
            GPIOE->BSRR = ((uint32_t(mask(4)) & ~inverted(4)) << 16) | inverted(4);
        if constexpr (mask(5))
            GPIOF->BSRR = ((uint32_t(mask(5)) & ~inverted(5)) << 16) | inverted(5);
        if constexpr (mask(6))
            GPIOG->BSRR = ((uint32_t(mask(6)) & ~inverted(6)) << 16) | inverted(6);
        if constexpr (mask(7))
            GPIOH->BSRR = ((uint32_t(mask(7)) & ~inverted(7)) << 16) | inverted(7);
        if constexpr (mask(8))
            GPIOI->BSRR = ((uint32_t(mask(8)) & ~inverted(8)) << 16) | inverted(8);
    }
    ...
    static void disconnect() { (Gpios::disconnect(), ...); }
};
