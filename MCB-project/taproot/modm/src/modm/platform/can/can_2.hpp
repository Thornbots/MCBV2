...
private:
    ...
    static bool initializeWithPrescaler(
        uint16_t prescaler,
        uint8_t bs1,
        uint8_t bs2,
...
public:
    ...
    static void connect(Gpio::InputType inputType = Gpio::InputType::Floating)
    {
        ...
        static_assert(
            Connector::template IsValid<Tx> and Connector::template IsValid<Rx> and
                sizeof...(Signals) == 2,
    ...
    [[nodiscard]] static inline bool initialize(
        uint32_t interruptPriority,
...
public:
     ...
     */
    static inline uint8_t getReceiveErrorCounter() { return (CAN2->ESR & CAN_ESR_REC) >> 24; }

    /**
     * Get Transmit Error Counter.
     *
     */
    static inline uint8_t getTransmitErrorCounter() { return (CAN2->ESR & CAN_ESR_TEC) >> 16; }
