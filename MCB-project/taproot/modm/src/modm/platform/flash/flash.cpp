...
uint8_t Flash::getPage(uintptr_t offset)
{
...
uint32_t Flash::getOffset(uint8_t index)
{
    switch (index)
    {
        case 0:
...
size_t Flash::getSize(uint8_t modm_unused index)
{
...
modm_ramcode uint32_t Flash::erase(uint8_t index, WordSize size)
{
...
modm_ramcode uint32_t Flash::program(uintptr_t addr, MaxWordType data, WordSize size)
{
    FLASH->SR = FLASH_SR_ERR;
