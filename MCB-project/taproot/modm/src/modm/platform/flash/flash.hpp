...
public:
    ...
    static bool isLocked() { return FLASH->CR & FLASH_CR_LOCK; }

    static inline bool isBusy() { return FLASH->SR & FLASH_SR_BSY; }
    static bool unlock();

    static inline uint8_t getSector(uint8_t *ptr) { return getPage(ptr - Flash::Origin); }

    static uint8_t getSector(uintptr_t offset) { return getPage(offset); }

    static inline uint8_t getPage(uint8_t *ptr) { return getPage(ptr - Flash::Origin); }

    static uint8_t getPage(uintptr_t offset);

    static inline uint8_t *getAddr(uint8_t sector) { return Origin + getOffset(sector); }
    ...
    static inline uint32_t program(uintptr_t addr, uint8_t data)
    {
        return program(addr, data, WordSize::B8);
    }

    static inline uint32_t program(uintptr_t addr, uint16_t data)
    {
