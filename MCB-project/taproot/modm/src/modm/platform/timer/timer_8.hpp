...
public:
    ...
    static inline void pause() { TIM8->CR1 &= ~TIM_CR1_CEN; }

    static inline void start() { TIM8->CR1 |= TIM_CR1_CEN; }
    ...
    {
        TIM8->CR2 = (TIM8->CR2 & ~TIM_CR2_CCUS) | static_cast<uint32_t>(update);
    };
    ...
    {
        TIM8->CR2 = (TIM8->CR2 & ~TIM_CR2_CCUS) | static_cast<uint32_t>(update) | TIM_CR2_CCPC;
    }

    static inline void disableCaptureComparePreloadedControl() { TIM8->CR2 &= ~TIM_CR2_CCPC; }
    ...
    static inline void setOverflow(uint16_t overflow) { TIM8->ARR = overflow; }
    ...
    static inline void generateEvent(Event ev) { TIM8->EGR |= static_cast<uint32_t>(ev); }

    static inline uint16_t getValue() { return TIM8->CNT; }

    static inline void setValue(uint16_t value) { TIM8->CNT = value; }

    static inline void enableOutput() { TIM8->BDTR |= TIM_BDTR_MOE; }

    static inline void disableOutput() { TIM8->BDTR &= ~(TIM_BDTR_MOE); }
...
public:
    ...
    static inline InterruptFlag getInterruptFlags() { return static_cast<InterruptFlag>(TIM8->SR); }
