...
// ----------------------------------------------------------------------------
void modm::platform::Timer1::enable() { Rcc::enable<Peripheral::Tim1>(); }

void modm::platform::Timer1::disable()
{
    TIM1->CR1 = 0;
    TIM1->DIER = 0;
    TIM1->CCER = 0;
...
// ----------------------------------------------------------------------------
void modm::platform::Timer1::setMode(
    Mode mode,
    SlaveMode slaveMode,
    ...
    MasterMode masterMode)
{
    ...
    TIM1->SMCR = static_cast<uint32_t>(slaveMode) | static_cast<uint32_t>(slaveModeTrigger);
}

// ----------------------------------------------------------------------------
void modm::platform::Timer1::configureInputChannel(
    uint32_t channel,
...
// ----------------------------------------------------------------------------
void modm::platform::Timer1::configureOutputChannel(
    uint32_t channel,
...
void modm::platform::Timer1::configureOutputChannel(
    ...
    PinState out_n,
    OutputComparePolarity polarity_n,
    OutputComparePreload preload)
{
    ...
    flags = (static_cast<uint32_t>(polarity_n) << 2) | (static_cast<uint32_t>(out_n) << 2) |
            static_cast<uint32_t>(polarity) | static_cast<uint32_t>(out);

    TIM1->CCER |= flags << (channel * 4);
}

void modm::platform::Timer1::configureOutputChannel(uint32_t channel, uint32_t modeOutputPorts)
{
    channel -= 1;  // 1..4 -> 0..3
...
// ----------------------------------------------------------------------------
void modm::platform::Timer1::enableInterruptVector(
    Interrupt interrupt,
    bool enable,
    uint32_t priority)
{
    ...
    if (interrupt & (Interrupt::CaptureCompare1 | Interrupt::CaptureCompare2 |
