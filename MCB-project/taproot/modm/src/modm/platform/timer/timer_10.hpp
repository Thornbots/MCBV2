...
#ifndef MODM_STM32_TIMER_10_HPP
#define MODM_STM32_TIMER_10_HPP
 ...
 * MODM_ISR(TIM10)
 * {
 *     Timer10::resetInterruptFlags(Timer10::...);
 *
 *     ...
 * }
 * @endcode
 ...
 */
class Timer10 : public GeneralPurposeTimer
{
public:
    ...
    enum class SlaveMode : uint32_t
    {
        ...
        /// Counter counts up/down on both TI1FP1 and TI2FP2 edges depending on the level of the
        /// other input.
        Encoder3 = TIM_SMCR_SMS_1 | TIM_SMCR_SMS_0,
        /// Rising edge of the selected trigger input (TRGI) reinitializes the counter and generates
        /// an update of the registers.
        Reset = TIM_SMCR_SMS_2,
        /// The counter clock is enabled when the trigger input (TRGI) is high. The counter stops
        /// (but is not reset) as soon as the trigger becomes low. Both start and stop of the
        /// counter are controlled.
        Gated = TIM_SMCR_SMS_2 | TIM_SMCR_SMS_0,
        /// The counter starts at a rising edge of the trigger TRGI (but it is not reset). Only the
    ...
    static void connect()
    {
        using Connector = GpioConnector<Peripheral::Tim10, Signals...>;
        Connector::connect();
    }
    ...
    static inline void pause() { TIM10->CR1 &= ~TIM_CR1_CEN; }

    static inline void start() { TIM10->CR1 |= TIM_CR1_CEN; }
    ...
    static uint16_t getPrescaler() { return (TIM10->PSC + 1); }

    static inline void setOverflow(Value overflow) { TIM10->ARR = overflow; }

    template <class SystemClock>
    static Value setPeriod(uint32_t microseconds, bool autoApply = true)
    {
        // This will be inaccurate for non-smooth frequencies (last six digits
        // unequal to zero)
        uint32_t cycles = microseconds * (SystemClock::Timer10 / 1'000'000UL);
    ...
    {
        return SystemClock::Timer10 / (TIM10->PSC + 1);
    }
    ...
    static inline Value getValue() { return TIM10->CNT; }

    static inline void setValue(Value value) { TIM10->CNT = value; }

    static inline void enableOutput() { TIM10->BDTR |= TIM_BDTR_MOE; }

    static inline void disableOutput() { TIM10->BDTR &= ~(TIM_BDTR_MOE); }
    ...
    {
        if (enable)
            TIM10->BDTR |= TIM_BDTR_AOE;
        else
            TIM10->BDTR &= ~TIM_BDTR_AOE;
    }

    static inline void setOffState(OffStateForRunMode runMode, OffStateForIdleMode idleMode)
    {
        uint32_t flags = TIM10->BDTR;
     ...
     *     111 => (DeadTime[4:0] + 2)  * 16 * T(DTS)
     */
    static inline void setDeadTime(DeadTimeResolution resolution, uint8_t deadTime)
    {
        uint8_t bitmask;
        switch (resolution)
        {
...
public:
    ...
    static void setInvertedPwm(uint32_t channel)
    {
...

        {
            ...
            if (channel <= 1)
            {
                ...
                flags <<= offset;
                flags |= TIM10->CCMR1 & ~(TIM_CCMR1_OC1M << offset);
                TIM10->CCMR1 = flags;
            }
            else
            {
                ...
                flags <<= offset;
                flags |= TIM10->CCMR2 & ~(TIM_CCMR1_OC1M << offset);
    ...
    static inline bool isChannelConfiguredAsInput([[maybe_unused]] uint32_t channel = 1)
    {
        return TIM10->CCMR1 & TIM_CCMR1_CC1S;
    }

    static inline void setCompareValue(uint32_t channel, Value value)
    {
        *(&TIM10->CCR1 + (channel - 1)) = value;
    }

    static inline Value getCompareValue(uint32_t channel)
    {
        return *(&TIM10->CCR1 + (channel - 1));
    }

public:
    static void enableInterruptVector(bool enable, uint32_t priority);

    static inline void enableInterrupt(Interrupt_t interrupt) { TIM10->DIER |= interrupt.value; }

    static inline void disableInterrupt(Interrupt_t interrupt) { TIM10->DIER &= ~interrupt.value; }

    static inline InterruptFlag_t getEnabledInterrupts() { return InterruptFlag_t(TIM10->DIER); }

    static inline void enableDmaRequest(DmaRequestEnable dmaRequests)
    {
        TIM10->DIER |= static_cast<uint32_t>(dmaRequests);
    }
    ...
    static inline InterruptFlag_t getInterruptFlags() { return InterruptFlag_t(TIM10->SR); }

    static inline void acknowledgeInterruptFlags(InterruptFlag_t flags)
    {
        // Flags are cleared by writing a zero to the flag position.
        // Writing a one is ignored.
        TIM10->SR = ~flags.value;
    }
};
