...
#ifndef MODM_STM32_TIMER_1_HPP
#define MODM_STM32_TIMER_1_HPP
...
public:
    ...
    static void connect()
    {
        using Connector = GpioConnector<Peripheral::Tim1, Signals...>;
        Connector::connect();
    }
    ...
    static inline void pause() { TIM1->CR1 &= ~TIM_CR1_CEN; }

    static inline void start() { TIM1->CR1 |= TIM_CR1_CEN; }
    ...
    {
        TIM1->CR2 = (TIM1->CR2 & ~TIM_CR2_CCUS) | static_cast<uint32_t>(update);
    };
    ...
    {
        TIM1->CR2 = (TIM1->CR2 & ~TIM_CR2_CCUS) | static_cast<uint32_t>(update) | TIM_CR2_CCPC;
    }

    static inline void disableCaptureComparePreloadedControl() { TIM1->CR2 &= ~TIM_CR2_CCPC; }
    ...
    static inline void setOverflow(uint16_t overflow) { TIM1->ARR = overflow; }
    ...
    static uint16_t setPeriod(uint32_t microseconds, bool autoApply = true)
    {
        ...
        // unequal to zero)
        uint32_t cycles = microseconds * (SystemClock::Timer1 / 1'000'000UL);
    ...
    static inline void generateEvent(Event ev) { TIM1->EGR |= static_cast<uint32_t>(ev); }

    static inline uint16_t getValue() { return TIM1->CNT; }

    static inline void setValue(uint16_t value) { TIM1->CNT = value; }

    static inline void enableOutput() { TIM1->BDTR |= TIM_BDTR_MOE; }

    static inline void disableOutput() { TIM1->BDTR &= ~(TIM_BDTR_MOE); }
    ...
    static inline void setOutputIdleState(
        uint32_t channel,
        OutputIdleState idle,
...
public:
    ...
    {
        TIM1->DIER |= static_cast<uint32_t>(interrupt);
    }
    ...
    {
        TIM1->DIER |= static_cast<uint32_t>(dmaRequests);
    }
    ...
    static inline InterruptFlag getInterruptFlags() { return static_cast<InterruptFlag>(TIM1->SR); }

    static inline void acknowledgeInterruptFlags(InterruptFlag flags)
    {
        // Flags are cleared by writing a zero to the flag position.
        // Writing a one is ignored.
        TIM1->SR = ~static_cast<uint32_t>(flags);
    }
};
