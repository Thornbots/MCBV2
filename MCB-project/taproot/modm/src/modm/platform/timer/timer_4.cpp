...
// ----------------------------------------------------------------------------
void modm::platform::Timer4::clockEnable() { enable(); }

void modm::platform::Timer4::enable() { Rcc::enable<Peripheral::Tim4>(); }

void modm::platform::Timer4::disable()
{
    TIM4->CR1 = 0;
    TIM4->DIER = 0;
    TIM4->CCER = 0;

    Rcc::disable<Peripheral::Tim4>();
}

// ----------------------------------------------------------------------------
void modm::platform::Timer4::setMode(
    Mode mode,
    SlaveMode slaveMode,
    SlaveModeTrigger slaveModeTrigger,
    MasterMode masterMode,
    bool enableOnePulseMode,
    bool bufferAutoReloadRegister,
    bool limitUpdateEventRequestSource)
{
    ...
    TIM4->SMCR = static_cast<uint32_t>(slaveMode) | static_cast<uint32_t>(slaveModeTrigger);
}

// ----------------------------------------------------------------------------
void modm::platform::Timer4::configureInputChannel(
    uint32_t channel,
    ...
    bool xor_ch1_3)
{
    ...
    TIM4->CCER |= (TIM_CCER_CC1E | static_cast<uint32_t>(polarity)) << (channel * 4);
}

// ----------------------------------------------------------------------------
void modm::platform::Timer4::configureOutputChannel(
    uint32_t channel,
...
// ----------------------------------------------------------------------------
void modm::platform::Timer4::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
// ----------------------------------------------------------------------------
bool modm::platform::Timer4::isChannelConfiguredAsInput(uint32_t channel)
{
    bool isInput = false;
    switch (channel)
    {
