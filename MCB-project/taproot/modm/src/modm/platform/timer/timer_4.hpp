...
public:
    ...
    static inline void pause() { TIM4->CR1 &= ~TIM_CR1_CEN; }

    static inline void start() { TIM4->CR1 |= TIM_CR1_CEN; }
    ...
    static uint16_t getPrescaler() { return (TIM4->PSC + 1); }

    static inline void setOverflow(Value overflow) { TIM4->ARR = overflow; }
    ...
    static inline Value getValue() { return TIM4->CNT; }

    static inline void setValue(Value value) { TIM4->CNT = value; }

    static inline void enableOutput() { TIM4->BDTR |= TIM_BDTR_MOE; }

    static inline void disableOutput() { TIM4->BDTR &= ~(TIM_BDTR_MOE); }
...
public:
    ...
    static inline Value getCompareValue(uint32_t channel) { return *(&TIM4->CCR1 + (channel - 1)); }

public:
    static void enableInterruptVector(bool enable, uint32_t priority);

    static inline void enableInterrupt(Interrupt_t interrupt) { TIM4->DIER |= interrupt.value; }

    static inline void disableInterrupt(Interrupt_t interrupt) { TIM4->DIER &= ~interrupt.value; }

    static inline InterruptFlag_t getEnabledInterrupts() { return InterruptFlag_t(TIM4->DIER); }
    ...
    static inline InterruptFlag_t getInterruptFlags() { return InterruptFlag_t(TIM4->SR); }
