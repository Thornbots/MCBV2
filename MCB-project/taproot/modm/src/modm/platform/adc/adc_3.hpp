 ...
 * @author	ekiwi
 * @ingroup	modm_platform_adc modm_platform_adc_3
 */
class Adc3 : public modm::Adc
{
public:
...
public:
    ...
    static void connect()
    {
        using Connector = GpioConnector<Peripheral::Adc3, Signals...>;
        Connector::connect();
    }
    ...
    static inline uint16_t readChannel(Channel channel);
     ...
     * 		initialize()
     */
    static inline bool setChannel(
        const Channel channel,
        const SampleTime sampleTime = static_cast<SampleTime>(0b000));

    /// Setting the channel for a Pin
    template <class Gpio>
    static inline bool setPinChannel(SampleTime sampleTime = static_cast<SampleTime>(0b000))
    {
    ...
    {
        constexpr int8_t channel{Gpio::template AdcChannel<Peripheral::Adc3>};
        static_assert(channel >= 0, "Adc3 does not have a channel for this pin!");
    ...
    static inline void setLeftAdjustResult();

    static inline void setRightAdjustResult();
    ...
    static inline bool addChannel(
        const Channel channel,
        const SampleTime sampleTime = static_cast<SampleTime>(0b000));

    /// change sample time of ADC channel
    static inline void setSampleTime(
        const Channel channel,
        const SampleTime sampleTime = static_cast<SampleTime>(0b000));

    static inline void enableInterruptVector(const uint32_t priority, const bool enable = true);
     ...
     * 	vector.
     */
    static inline void enableInterrupt(const Interrupt_t interrupt);

    /**
     * Disables the ADC Conversion Complete Interrupt.
     */
    static inline void disableInterrupt(const Interrupt_t interrupt);
...
#include "adc_3_impl.hpp"

#endif  // MODM_STM32_ADC3_HPP