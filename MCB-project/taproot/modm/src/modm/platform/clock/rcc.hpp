...
public:
    ...
    enum class RealTimeClockSource : uint32_t
    {
    ...
    enum class WatchdogClockSource : uint32_t
    {
    ...
    enum class ClockOutput1Source : uint32_t
    {
        InternalClock = 0,
    ...
    enum class ClockOutput2Source : uint32_t
    {
        SystemClock = 0,
...
public:
     ...
     *
     */
    [[deprecated("Use PllFactors as argument instead")]] static bool enablePll(
        PllSource source,
        uint8_t pllM,
        uint16_t pllN,
        uint8_t pllP,
    ...
    static inline bool enableRealTimeClock(RealTimeClockSource src)
    {
        RCC->BDCR = (RCC->BDCR & ~RCC_BDCR_RTCSEL) | RCC_BDCR_RTCEN | uint32_t(src);
        return true;
    }

    static inline bool enableWatchdogClock(WatchdogClockSource /*src*/) { return true; }

    static inline bool enableClockOutput1(ClockOutput1Source src, uint8_t div)
    {
    ...
    static inline bool enableClockOutput2(ClockOutput2Source src, uint8_t div)
    {
...
public:
    static inline bool setAhbPrescaler(AhbPrescaler prescaler)
    {
        RCC->CFGR = (RCC->CFGR & ~RCC_CFGR_HPRE) | uint32_t(prescaler);
        return true;
    }

    static inline bool setApb1Prescaler(Apb1Prescaler prescaler)
    {
        RCC->CFGR = (RCC->CFGR & ~RCC_CFGR_PPRE1) | uint32_t(prescaler);
        return true;
    }

    static inline bool setApb2Prescaler(Apb2Prescaler prescaler)
    {
...
private:
    ...
    static constexpr flash_latency computeFlashLatency(uint32_t Core_Hz, uint16_t Core_mV);
};
