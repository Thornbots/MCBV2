...
// ----------------------------------------------------------------------------
void UsartHal3::enable() { Rcc::enable<Peripheral::Usart3>(); }

void UsartHal3::disable()
{
...
void UsartHal3::enableOperation() { USART3->CR1 |= USART_CR1_UE; }

void UsartHal3::disableOperation() { USART3->CR1 &= ~USART_CR1_UE; }

template <class SystemClock, modm::baudrate_t baudrate, modm::percent_t tolerance>
void UsartHal3::initialize(Parity parity, WordLength length)
{
    enable();
    disableOperation();
...
void UsartHal3::setSpiClock(SpiClock clk, LastBitClockPulse pulse)
{
...
void UsartHal3::setSpiDataMode(SpiDataMode mode)
{
    uint32_t cr2 = USART3->CR2;
...
void UsartHal3::write(uint16_t data) { USART3->DR = data; }

void UsartHal3::read(uint8_t &data) { data = USART3->DR; }

void UsartHal3::read(uint16_t &data) { data = USART3->DR; }

void UsartHal3::setTransmitterEnable(bool enable)
{
    if (enable)
    {
...
void UsartHal3::setReceiverEnable(bool enable)
{
    if (enable)
    {
...
bool UsartHal3::isReceiveRegisterNotEmpty() { return USART3->SR & USART_SR_RXNE; }

bool UsartHal3::isTransmitRegisterEmpty() { return USART3->SR & USART_SR_TXE; }

void UsartHal3::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
void UsartHal3::setInterruptPriority(uint32_t priority) { NVIC_SetPriority(USART3_IRQn, priority); }

void UsartHal3::enableInterrupt(Interrupt_t interrupt) { USART3->CR1 |= interrupt.value; }

void UsartHal3::disableInterrupt(Interrupt_t interrupt) { USART3->CR1 &= ~interrupt.value; }

UsartHal3::InterruptFlag_t UsartHal3::getInterruptFlags() { return InterruptFlag_t(USART3->SR); }

void UsartHal3::acknowledgeInterruptFlags(InterruptFlag_t flags)
{
