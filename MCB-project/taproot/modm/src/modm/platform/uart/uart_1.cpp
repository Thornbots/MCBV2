...
void Usart1::writeBlocking(uint8_t data)
{
...
void Usart1::writeBlocking(const uint8_t *data, std::size_t length)
{
    while (length-- != 0)
    {
        writeBlocking(*data++);
...
std::size_t Usart1::write(const uint8_t *data, std::size_t length)
{
    uint32_t i = 0;
    for (; i < length; ++i)
    {
...
bool Usart1::isWriteFinished()
{
    return txBuffer.isEmpty() && UsartHal1::isTransmitRegisterEmpty();
}

std::size_t Usart1::transmitBufferSize() { return txBuffer.getSize(); }

std::size_t Usart1::discardTransmitBuffer()
{
    std::size_t count = 0;
...
std::size_t Usart1::read(uint8_t *data, std::size_t length)
{
    uint32_t i = 0;
    for (; i < length; ++i)
    {
...
std::size_t Usart1::receiveBufferSize() { return rxBuffer.getSize(); }

std::size_t Usart1::discardReceiveBuffer()
{
    std::size_t count = 0;
