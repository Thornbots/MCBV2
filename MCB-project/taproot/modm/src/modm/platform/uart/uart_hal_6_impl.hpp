...
// ----------------------------------------------------------------------------
void UsartHal6::enable() { Rcc::enable<Peripheral::Usart6>(); }

void UsartHal6::disable()
{
...
void UsartHal6::enableOperation() { USART6->CR1 |= USART_CR1_UE; }

void UsartHal6::disableOperation() { USART6->CR1 &= ~USART_CR1_UE; }

template <class SystemClock, modm::baudrate_t baudrate, modm::percent_t tolerance>
void UsartHal6::initialize(Parity parity, WordLength length)
{
    enable();
    disableOperation();
...
void UsartHal6::setSpiClock(SpiClock clk, LastBitClockPulse pulse)
{
...
void UsartHal6::setSpiDataMode(SpiDataMode mode)
{
    uint32_t cr2 = USART6->CR2;
...
void UsartHal6::write(uint16_t data) { USART6->DR = data; }

void UsartHal6::read(uint8_t &data) { data = USART6->DR; }

void UsartHal6::read(uint16_t &data) { data = USART6->DR; }

void UsartHal6::setTransmitterEnable(bool enable)
{
    if (enable)
    {
...
void UsartHal6::setReceiverEnable(bool enable)
{
    if (enable)
    {
...
bool UsartHal6::isReceiveRegisterNotEmpty() { return USART6->SR & USART_SR_RXNE; }

bool UsartHal6::isTransmitRegisterEmpty() { return USART6->SR & USART_SR_TXE; }

void UsartHal6::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
void UsartHal6::setInterruptPriority(uint32_t priority) { NVIC_SetPriority(USART6_IRQn, priority); }

void UsartHal6::enableInterrupt(Interrupt_t interrupt) { USART6->CR1 |= interrupt.value; }

void UsartHal6::disableInterrupt(Interrupt_t interrupt) { USART6->CR1 &= ~interrupt.value; }

UsartHal6::InterruptFlag_t UsartHal6::getInterruptFlags() { return InterruptFlag_t(USART6->SR); }

void UsartHal6::acknowledgeInterruptFlags(InterruptFlag_t flags)
{
