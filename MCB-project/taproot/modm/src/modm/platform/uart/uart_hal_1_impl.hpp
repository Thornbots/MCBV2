...
// ----------------------------------------------------------------------------
void UsartHal1::enable() { Rcc::enable<Peripheral::Usart1>(); }

void UsartHal1::disable()
{
...
void UsartHal1::enableOperation() { USART1->CR1 |= USART_CR1_UE; }

void UsartHal1::disableOperation() { USART1->CR1 &= ~USART_CR1_UE; }

template <class SystemClock, modm::baudrate_t baudrate, modm::percent_t tolerance>
void UsartHal1::initialize(Parity parity, WordLength length)
{
    enable();
    disableOperation();
...
void UsartHal1::setSpiClock(SpiClock clk, LastBitClockPulse pulse)
{
...
void UsartHal1::setSpiDataMode(SpiDataMode mode)
{
    uint32_t cr2 = USART1->CR2;
...
void UsartHal1::write(uint16_t data) { USART1->DR = data; }

void UsartHal1::read(uint8_t &data) { data = USART1->DR; }

void UsartHal1::read(uint16_t &data) { data = USART1->DR; }

void UsartHal1::setTransmitterEnable(bool enable)
{
    if (enable)
    {
...
void UsartHal1::setReceiverEnable(bool enable)
{
    if (enable)
    {
...
bool UsartHal1::isReceiveRegisterNotEmpty() { return USART1->SR & USART_SR_RXNE; }

bool UsartHal1::isTransmitRegisterEmpty() { return USART1->SR & USART_SR_TXE; }

void UsartHal1::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
void UsartHal1::setInterruptPriority(uint32_t priority) { NVIC_SetPriority(USART1_IRQn, priority); }

void UsartHal1::enableInterrupt(Interrupt_t interrupt) { USART1->CR1 |= interrupt.value; }

void UsartHal1::disableInterrupt(Interrupt_t interrupt) { USART1->CR1 &= ~interrupt.value; }

UsartHal1::InterruptFlag_t UsartHal1::getInterruptFlags() { return InterruptFlag_t(USART1->SR); }

void UsartHal1::acknowledgeInterruptFlags(InterruptFlag_t flags)
{
