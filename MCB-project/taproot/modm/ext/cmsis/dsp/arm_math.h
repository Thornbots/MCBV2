...
/**
   ...
   * ------------
   *
   * The library installer contains prebuilt versions of the libraries in the <code>Lib</code>
   folder.
   *
   ...
   * - arm_ARMv8MMLlfsp_math.lib (Armv8-M Mainline, Little endian, Single Precision Floating Point
   Unit)
   * - arm_ARMv8MMLld_math.lib (Armv8-M Mainline, Little endian, DSP instructions)
   * - arm_ARMv8MMLldfsp_math.lib (Armv8-M Mainline, Little endian, DSP instructions, Single
   ...
   * Simply include this file and link the appropriate library in the application and begin calling
   the library functions. The Library supports single
   * public header file <code> arm_math.h</code> for Cortex-M cores with little endian and big
   endian. Same header file will be used for floating point unit(FPU) variants.
   *
   *
   * Examples
   * --------
   ...
   *
   *
   * The libraries can be built by opening the arm_cortexM_math.uvprojx project in MDK-ARM,
   selecting a specific target, and defining the optional preprocessor macros detailed above.
   *
...
#define SQ(x) ((x) * (x))
    ...
    __STATIC_FORCEINLINE q31_t read_q7x4_da(q7_t **pQ7)
    {
        ...
        memcpy(&val, *pQ7, 4);
#else
    val = ((((*pQ7)[3]) & 0x0FF) << 24) | ((((*pQ7)[2]) & 0x0FF) << 16) |
          ((((*pQ7)[1]) & 0x0FF) << 8) | ((*pQ7)[0] & 0x0FF);
#endif
        *pQ7 -= 4;
...
#define __PACKq7(v0, v1, v2, v3)                                                                   \
    ...
    {
        return ((q31_t)(x >> 32) != ((q31_t)x >> 31)) ? ((0x7FFFFFFF ^ ((q31_t)(x >> 63))))
                                                      : (q31_t)x;
    }
 ...
 */
    ...
    {
        /*  q31_t r,     s;  without initialisation 'arm_offset_q15 test' fails  but 'intrinsic'
         * tests pass! for armCC */
        q31_t r = 0, s = 0;
    ...
    {
        return ((
            uint32_t)(((((q31_t)x << 16) >> 16) * (((q31_t)y << 16) >> 16)) + ((((q31_t)x) >> 16) * (((q31_t)y) >> 16)) + (((q31_t)sum))));
    }
    ...
    {
        return ((
            uint32_t)(((((q31_t)x << 16) >> 16) * (((q31_t)y) >> 16)) + ((((q31_t)x) >> 16) * (((q31_t)y << 16) >> 16)) + (((q31_t)sum))));
    }
    ...
    __STATIC_FORCEINLINE uint64_t __SMLALD(uint32_t x, uint32_t y, uint64_t sum)
    {
        ...
        return ((
            uint64_t)(((((q31_t)x << 16) >> 16) * (((q31_t)y << 16) >> 16)) + ((((q31_t)x) >> 16) * (((q31_t)y) >> 16)) + (((q63_t)sum))));
    }
    ...
    __STATIC_FORCEINLINE uint64_t __SMLALDX(uint32_t x, uint32_t y, uint64_t sum)
    {
        ...
        return ((
            uint64_t)(((((q31_t)x << 16) >> 16) * (((q31_t)y) >> 16)) + ((((q31_t)x) >> 16) * (((q31_t)y << 16) >> 16)) + (((q63_t)sum))));
    }
     ...
     * @param[in]     numTaps    Number of filter coefficients in the filter. Must be even and
    ...
    typedef struct
    {
        int8_t numStages; /**< number of 2nd order stages in the filter.  Overall order is
                             2*numStages. */
        q15_t *pState;    /**< Points to the array of state coefficients.  The array is of length
                             4*numStages. */
        const q15_t *pCoeffs; /**< Points to the array of coefficients.  The array is of length
                                 5*numStages. */
    ...
    typedef struct
    {
        uint32_t numStages; /**< number of 2nd order stages in the filter.  Overall order is
                               2*numStages. */
        q31_t *pState;      /**< Points to the array of state coefficients.  The array is of length
                               4*numStages. */
        const q31_t *pCoeffs; /**< Points to the array of coefficients.  The array is of length
                                 5*numStages. */
    ...
    typedef struct
    {
        uint32_t numStages; /**< number of 2nd order stages in the filter.  Overall order is
                               2*numStages. */
        float32_t *pState;  /**< Points to the array of state coefficients.  The array is of length
                               4*numStages. */
        const float32_t *pCoeffs; /**< Points to the array of coefficients.  The array is of length
                                     5*numStages. */
    ...
    typedef struct
    {
        float32_t coeffs[8][4]; /**< Points to the array of modified coefficients.  The array is of
                                   length 32. There is one per stage */
     ...
     * @param[in]     pCoeffs    points to the filter coefficients.
     * @param[in]     pState     points to the state buffer.
     * @param[in]     postShift  Shift to be applied to the output. Varies according to the
     * coefficients format
     */
    ...
    /**
     * @brief Fast but less precise processing function for the Q15 Biquad cascade filter for
    ...
    /**
     * @brief Fast but less precise processing function for the Q31 Biquad cascade filter for
     * Cortex-M3 and Cortex-M4.
     * @param[in]  S          points to an instance of the Q31 Biquad cascade structure.
     * @param[in]  pSrc       points to the block of input data.
     * @param[out] pDst       points to the block of output data.
     ...
     * @param[in,out] S          points to an instance of the floating-point Biquad cascade
     * structure.
     * @param[in]     numStages  number of 2nd order stages in the filter.
     * @param[in]     pCoeffs    points to the filter coefficients.
     ...
     * @return     The function returns either
     * <code>ARM_MATH_SIZE_MISMATCH</code> or <code>ARM_MATH_SUCCESS</code> based on the outcome of
     * size checking.
     */
     ...
     * @param[in]     resetStateFlag  flag to reset the state. 0 = no change in state 1 = reset the
     * state.
     */
    ...
    typedef struct
    {
        ...
        uint8_t ifftFlag;       /**< flag that selects forward (ifftFlag=0) or inverse (ifftFlag=1)
                                   transform. */
        ...
        uint16_t twidCoefModifier;    /**< twiddle coefficient modifier that supports different size
                                         FFTs with the same twiddle factor table. */
    ...
    typedef struct
    {
        ...
        const uint32_t *rearranged_twiddle_tab_stride1_arr; /**< Per stage reordered twiddle pointer
                                                               (offset 1) */
        const uint32_t *rearranged_twiddle_tab_stride2_arr; /**< Per stage reordered twiddle pointer
                                                               (offset 2) */
        const uint32_t *rearranged_twiddle_tab_stride3_arr; /**< Per stage reordered twiddle pointer
                                                               (offset 3) */
    ...
    typedef struct
    {
        ...
        uint8_t ifftFlagR;   /**< flag that selects forward (ifftFlagR=0) or inverse (ifftFlagR=1)
                                transform. */
        uint8_t bitReverseFlagR;    /**< flag that enables (bitReverseFlagR=1) or disables
                                       (bitReverseFlagR=0) bit reversal of output. */
        uint32_t twidCoefRModifier; /**< twiddle coefficient modifier that supports different size
                                       FFTs with the same twiddle factor table. */
     ...
     * @param[in]     S              points to an instance of the floating-point DCT4/IDCT4
     * structure.
     ...
     * @param[in]  pSrc       points to the input vector
     * @param[in]  shiftBits  number of bits to shift.  A positive value shifts left; a negative
     * value shifts right.
     * @param[out] pDst       points to the output vector
     * @param[in]  blockSize  number of samples in the vector
     */
     ...
     * @param[out] pDst     points to the location where the output result is written.  Length
     * srcALen+srcBLen-1.
     */
    void arm_conv_f32(
        const float32_t *pSrcA,
     ...
     * @param[in]  srcBLen    length of the second input sequence.
     * @param[out] pDst       points to the block of output data  Length srcALen+srcBLen-1.
     * @param[in]  pScratch1  points to scratch buffer(of type q15_t) of size max(srcALen, srcBLen)
     * + 2*min(srcALen, srcBLen) - 2.
     * @param[in]  pScratch2  points to scratch buffer (of type q15_t) of size min(srcALen,
     * srcBLen).
     */
     ...
     * @return  Returns either ARM_MATH_SUCCESS if the function completed correctly or
     * ARM_MATH_ARGUMENT_ERROR if the requested subset is not in the range [0 srcALen+srcBLen-2].
     */
    ...
    /**
      ...
      @return        execution status
                       ...
                       - \ref ARM_MATH_LENGTH_ERROR : <code>blockSize</code> is not a multiple of
      <code>M</code>
     */
    ...
    /**
     * @brief Processing function for the Q15 FIR decimator (fast variant) for Cortex-M3 and
     * Cortex-M4.
     * @param[in]  S          points to an instance of the Q15 FIR decimator structure.
     * @param[in]  pSrc       points to the block of input data.
     * @param[out] pDst       points to the block of output data
     ...
     * @param[in]     pState     points to the state buffer.
     * @param[in]     blockSize  number of input samples to process per call.
     * @return    The function returns ARM_MATH_SUCCESS if initialization is successful or
     * ARM_MATH_LENGTH_ERROR if <code>blockSize</code> is not a multiple of <code>M</code>.
     */
    ...
    /**
     * @brief Processing function for the Q31 FIR decimator (fast variant) for Cortex-M3 and
     * Cortex-M4.
     * @param[in]  S          points to an instance of the Q31 FIR decimator structure.
     * @param[in]  pSrc       points to the block of input data.
     * @param[out] pDst       points to the block of output data
     ...
     * @param[in]  S          points to an instance of the floating-point FIR interpolator
     * structure.
     * @param[in]  pSrc       points to the block of input data.
     * @param[out] pDst       points to the block of output data.
     * @param[in]  blockSize  number of input samples to process per call.
     ...
     * @param[in,out] S          points to an instance of the floating-point FIR interpolator
     * structure.
     * @param[in]     L          upsample factor.
     * @param[in]     numTaps    number of filter coefficients in the filter.
     * @param[in]     pCoeffs    points to the filter coefficient buffer.
    ...
    typedef struct
    {
        uint8_t numStages; /**< number of 2nd order stages in the filter.  Overall order is
                              2*numStages. */
        q63_t *pState;     /**< points to the array of state coefficients.  The array is of length
                              4*numStages. */
        const q31_t *pCoeffs; /**< points to the array of coefficients.  The array is of length
                                 5*numStages. */
        uint8_t postShift;    /**< additional shift, in bits, applied to each output sample. */
    ...
    /**
     * @brief Instance structure for the floating-point transposed direct form II Biquad cascade
     * filter.
     */
    typedef struct
    {
    ...
    typedef struct
    {
        uint8_t numStages; /**< number of 2nd order stages in the filter.  Overall order is
                              2*numStages. */
        float64_t *pState; /**< points to the array of state coefficients.  The array is of length
                              2*numStages. */
        const float64_t *pCoeffs; /**< points to the array of coefficients.  The array is of length
                                     5*numStages. */
    ...
    typedef struct
    {
        ...
        q15_t *pState;      /**< points to the state variable array. The array is of length
                               numStages+blockSize. */
        q15_t *pkCoeffs;    /**< points to the reflection coefficient array. The array is of length
                               numStages. */
        q15_t *pvCoeffs;    /**< points to the ladder coefficient array. The array is of length
                               numStages+1. */
    ...
    typedef struct
    {
        ...
        q31_t *pState;      /**< points to the state variable array. The array is of length
                               numStages+blockSize. */
        q31_t *pkCoeffs;    /**< points to the reflection coefficient array. The array is of length
                               numStages. */
        q31_t *pvCoeffs;    /**< points to the ladder coefficient array. The array is of length
                               numStages+1. */
    ...
    typedef struct
    {
        ...
        float32_t *pState;   /**< points to the state variable array. The array is of length
                                numStages+blockSize. */
        float32_t *pkCoeffs; /**< points to the reflection coefficient array. The array is of length
                                numStages. */
        float32_t *pvCoeffs; /**< points to the ladder coefficient array. The array is of length
                                numStages+1. */
     ...
     * @param[in] pkCoeffs   points to the reflection coefficient buffer.  The array is of length
     * numStages.
     * @param[in] pvCoeffs   points to the ladder coefficient buffer.  The array is of length
     * numStages+1.
     ...
     * @param[in] numStages  number of stages in the filter.
     * @param[in] pkCoeffs   points to reflection coefficient buffer.  The array is of length
     * numStages.
     * @param[in] pvCoeffs   points to ladder coefficient buffer.  The array is of length
     * numStages+1.
     ...
     * @param[in]  S          points to an instance of the floating-point normalized LMS filter
     * structure.
     * @param[in]  pSrc       points to the block of input data.
     * @param[in]  pRef       points to the block of reference data.
     * @param[out] pOut       points to the block of output data.
     ...
     * @param[in]  pSrcB    points to the second input sequence.
     * @param[in]  srcBLen  length of the second input sequence.
     * @param[out] pDst     points to the block of output data  Length 2 * max(srcALen, srcBLen)
     * - 1.
     */
    ...
    /**
     ...
     @param[in]  pScratch  points to scratch buffer of size max(srcALen, srcBLen) + 2*min(srcALen,
     srcBLen) - 2.
    */
    ...
    /**
      ...
      @param[in]     pSrcB      points to the second input sequence
      @param[in]     srcBLen    length of the second input sequence
      @param[out]    pDst       points to the location where the output result is written.  Length 2
    ...
    {
        uint16_t numTaps;    /**< number of coefficients in the filter. */
        uint16_t stateIndex; /**< state buffer index.  Points to the oldest sample in the state
                                buffer. */
        float32_t *pState;   /**< points to the state buffer array. The array is of length
     ...
     *
     * \par
     * where \c Kp is proportional constant, \c Ki is Integral constant and \c Kd is Derivative
     * constant
     *
     * \par
     ...
     * The Gains A0, A1, A2 and state variables for a PID controller are stored together in an
     * instance data structure. A separate instance structure must be defined for each PID
     * Controller. There are separate instance structure declarations for each of the 3 supported
     * data types.
     *
     * \par Reset Functions
     ...
     *
     * \par
     * Instance structure cannot be placed into a const data section and it is recommended to use
     * the initialization function.
     *
     * \par Fixed-Point Behavior
     * Care must be taken when using the fixed-point versions of the PID Controller functions.
     * In particular, the overflow and saturation behavior of the accumulator used in each function
    ...
    /**
      ...
      multiplication results but provides only a single guard bit. Thus, if the accumulator result
      overflows it wraps around rather than clip. In order to avoid overflows completely the input
      signal must be scaled down by 2 bits as there are four additions. After all
      multiply-accumulates are performed, the 2.62 accumulator is truncated to 1.32 format and then
    ...
    /**
      ...
      \par Scaling and Overflow Behavior
             ...
             Both Gains and state variables are represented in 1.15 format and multiplications yield
      a 2.30 result. The 2.30 intermediate results are accumulated in a 64-bit accumulator in 34.30
      format. There is no risk of internal overflow with this approach and the full precision of
      intermediate multiplications is preserved. After all additions have been performed, the
      accumulator is truncated to 34.15 format by discarding low 15 bits. Lastly, the accumulator is
     ...
     * If the input matrix is singular (does not have an inverse), then the algorithm terminates and
     * returns error status ARM_MATH_SINGULAR.
     */
     ...
     * Forward Clarke transform converts the instantaneous stator phases into a two-coordinate time
     * invariant vector. Generally the Clarke transform uses three-phase currents <code>Ia, Ib and
     * Ic</code> to calculate currents in the two-phase orthogonal stator axis <code>Ialpha</code>
     * and <code>Ibeta</code>. When <code>Ialpha</code> is superposed with <code>Ia</code> as shown
     * in the figure below \image html clarke.gif Stator current space vector and its components in
     * (a,b). and <code>Ia + Ib + Ic = 0</code>, in this condition <code>Ialpha</code> and
     * <code>Ibeta</code> can be calculated using only <code>Ia</code> and <code>Ib</code>.
     *
     * The function operates on a single sample of data and each call to the function returns the
     * processed output. The library provides separate functions for Q31 and floating-point data
     * types. \par Algorithm \image html clarkeFormula.gif where <code>Ia</code> and <code>Ib</code>
     * are the instantaneous stator phases and <code>pIalpha</code> and <code>pIbeta</code> are the
     * two coordinates of time invariant vector. \par Fixed-Point Behavior Care must be taken when
     * using the Q31 version of the Clarke transform. In particular, the overflow and saturation
     * behavior of the accumulator used must be considered. Refer to the function specific
    ...
    /**
      ...
      \par Scaling and Overflow Behavior
             ...
             The accumulator maintains 1.31 format by truncating lower 31 bits of the intermediate
      multiplication in 2.62 format. There is saturation on the addition, hence there is no risk of
      overflow.
     */
     ...
     * Inverse Clarke transform converts the two-coordinate time invariant vector into instantaneous
     * stator phases.
     *
     * The function operates on a single sample of data and each call to the function returns the
     * processed output. The library provides separate functions for Q31 and floating-point data
     * types. \par Algorithm \image html clarkeInvFormula.gif where <code>pIa</code> and
     * <code>pIb</code> are the instantaneous stator phases and <code>Ialpha</code> and
     * <code>Ibeta</code> are the two coordinates of time invariant vector. \par Fixed-Point
     * Behavior Care must be taken when using the Q31 version of the Clarke transform. In
     * particular, the overflow and saturation behavior of the accumulator used must be considered.
        ...
        float32_t *pIb)
    {
        ...
        *pIa = Ialpha;
        ...
        /* Calculating pIb from Ialpha and Ibeta by equation pIb = -(1/2) * Ialpha + (sqrt(3)/2) *
         * Ibeta */
    ...
    /**
      ...
      \par Scaling and Overflow Behavior
             The function is implemented using an internal 32-bit accumulator.
             The accumulator maintains 1.31 format by truncating lower 31 bits of the intermediate
      multiplication in 2.62 format. There is saturation on the subtraction, hence there is no risk
      of overflow.
     */
     ...
     *
     * Forward Park transform converts the input two-coordinate vector to flux and torque
     * components. The Park transform can be used to realize the transformation of the
     * <code>Ialpha</code> and the <code>Ibeta</code> currents from the stationary to the moving
     * reference frame and control the spatial relationship between the stator vector current and
     * rotor flux vector. If we consider the d axis aligned with the rotor flux, the diagram below
     * shows the current vector and the relationship from the two reference frames: \image html
     * park.gif "Stator current space vector and its component in (a,b) and in the d,q rotating
     * reference frame"
     *
     * The function operates on a single sample of data and each call to the function returns the
     * processed output. The library provides separate functions for Q31 and floating-point data
     * types. \par Algorithm \image html parkFormula.gif where <code>Ialpha</code> and
     * <code>Ibeta</code> are the stator vector components, <code>pId</code> and <code>pIq</code>
     * are rotor vector components and <code>cosVal</code> and <code>sinVal</code> are the cosine
     * and sine values of theta (rotor flux position). \par Fixed-Point Behavior Care must be taken
     * when using the Q31 version of the Park transform. In particular, the overflow and saturation
     * behavior of the accumulator used must be considered. Refer to the function specific
     * documentation below for usage guidelines.
     */
    ...
    /**
      ...
      \par Scaling and Overflow Behavior
             The function is implemented using an internal 32-bit accumulator.
             The accumulator maintains 1.31 format by truncating lower 31 bits of the intermediate
      multiplication in 2.62 format. There is saturation on the addition and subtraction, hence
     ...
     * Inverse Park transform converts the input flux and torque components to two-coordinate
     * vector.
     *
     * The function operates on a single sample of data and each call to the function returns the
     * processed output. The library provides separate functions for Q31 and floating-point data
     * types. \par Algorithm \image html parkInvFormula.gif where <code>pIalpha</code> and
     * <code>pIbeta</code> are the stator vector components, <code>Id</code> and <code>Iq</code> are
     * rotor vector components and <code>cosVal</code> and <code>sinVal</code> are the cosine and
     * sine values of theta (rotor flux position). \par Fixed-Point Behavior Care must be taken when
     * using the Q31 version of the Park transform. In particular, the overflow and saturation
     * behavior of the accumulator used must be considered. Refer to the function specific
     * documentation below for usage guidelines.
     */
     ...
     * Linear interpolation works by effectively drawing a straight line between two neighboring
     ...
     * using linear interpolation of the input values x0, x1( nearest input values) and the output
     * values y0 and y1(nearest output values)
     *
     * \par Algorithm:
     * <pre>
     ...
     *
     * \par
     * Input sample <code>x</code> is in 12.20 format which contains 12 bits for table index and 20
     * bits for fractional part. This function can support maximum of table size 2^12.
     *
     */
     ...
     * @return The function returns ARM_MATH_SUCCESS if input value is positive value or
     * ARM_MATH_ARGUMENT_ERROR if <code>in</code> is negative value and returns zero output for
     * negative values.
     */
    ...
    void arm_svm_polynomial_init_f32(
        arm_svm_polynomial_instance_f32 *S,
        uint32_t nbOfSupportVectors,
        uint32_t vectorDimension,
        float32_t intercept,
        const float32_t *dualCoefficients,
    ...
    void arm_svm_rbf_init_f32(
        arm_svm_rbf_instance_f32 *S,
        uint32_t nbOfSupportVectors,
        uint32_t vectorDimension,
        float32_t intercept,
        const float32_t *dualCoefficients,
    ...
    void arm_svm_sigmoid_init_f32(
        arm_svm_sigmoid_instance_f32 *S,
        uint32_t nbOfSupportVectors,
        uint32_t vectorDimension,
        float32_t intercept,
        const float32_t *dualCoefficients,
     ...
     *
     */
    float32_t arm_weighted_sum_f32(
        const float32_t *in,
    ...
    float32_t arm_euclidean_distance_f32(
        const float32_t *pA,
        const float32_t *pB,
        uint32_t blockSize);
     ...
     *
     */
    float32_t arm_braycurtis_distance_f32(
        const float32_t *pA,
        const float32_t *pB,
        uint32_t blockSize);
     ...
     *
     */
    float32_t arm_canberra_distance_f32(
        const float32_t *pA,
        const float32_t *pB,
        uint32_t blockSize);
     ...
     *
     */
    float32_t arm_chebyshev_distance_f32(
        const float32_t *pA,
        const float32_t *pB,
        uint32_t blockSize);
     ...
     *
     */
    float32_t arm_cityblock_distance_f32(
        const float32_t *pA,
        const float32_t *pB,
        uint32_t blockSize);
     ...
     *
     * Bilinear interpolation is an extension of linear interpolation applied to a two dimensional
     * grid. The underlying function <code>f(x, y)</code> is sampled on a regular grid and the
     * interpolation process determines values between the grid points. Bilinear interpolation is
     * equivalent to two step linear interpolation, first in the x-dimension and then in the
     * y-dimension. Bilinear interpolation is often used in image processing to rescale images. The
     * CMSIS DSP library provides bilinear interpolation functions for Q7, Q15, Q31, and
     * floating-point data types.
     *
     * <b>Algorithm</b>
     * \par
     * The instance structure used by the bilinear interpolation functions describes a two
     * dimensional data table. For floating-point, the instance structure is defined as: <pre>
     *   typedef struct
     *   {
     *     uint16_t numRows;
     *     uint16_t numCols;
     ...
     * The data table <code>pTable</code> is organized in row order and the supplied data values
     * fall on integer indexes. That is, table element (x,y) is located at <code>pTable[x +
